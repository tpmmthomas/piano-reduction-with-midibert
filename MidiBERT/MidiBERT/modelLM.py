...
class MidiBertLM(nn.Module):
    def __init__(self, midibert):
        super().__init__()

        self.midibert = midibert
        self.mask_lm = MLM(
            self.midibert.e2w, self.midibert.n_tokens, self.midibert.hidden_size
        )
...
class MidiBertSeq2SeqComplete(nn.Module):
    def __init__(self, midibert):
        super().__init__()

        self.midibert = midibert
        self.mask_lm = InverseEmbedding(
            self.midibert.e2w, self.midibert.n_tokens, self.midibert.hidden_size
        )

    def forward(self, x, y, attn_en, attn_de):
        x = self.midibert(x, y, attn_en, attn_de)
        return self.mask_lm(x)
